		...
		<div class="registerPage">
			...
			<el-form :model="registerForm" :rules="registerRules" ref="registerForm" status-icon>
				...
				<el-form-item prop="userPass">
				    <el-input v-model="registerForm.userPass" placeholder="请输入密码(6-12位数字和字母)"></el-input>
				</el-form-item>
				<el-form-item>
				    <el-button type="danger" @click="toRegister('registerForm')">注册</el-button>
				</el-form-item>
			</el-form>
			<p class="tip">已有账号？去<span @click="toLogin">登录</span></p>
		</div>
	</div>
</template>
<style scoped>
	.box{position: absolute;top:0;bottom: 0;left:0;right:0;overflow:hidden;}
	.registerPage{width:400px;top:200px;left:50%;margin-left:-200px; position:absolute;background-color: rgba(255,255,255,0.3);padding:20px;border-radius: 8px;}
	.title{text-align: center;color:#fff;font-size: 26px;padding:20px;font-weight: 400;}
	.el-form{width:100%}
	.el-button{width:100%;}
	.tip{text-align: center;color:#fff;}
	.tip span{cursor:pointer;color:#f56c6c;padding-left:3px;}
	/*背景动画*/
	.background {
	  width: 100vw;
	  height: 100vh;
...
<script>
	export default{
		data (){
			var validatPhone=(rule,value,cb)=>{
				var phoneReg=/^[1][3,4,5,7,8][0,9]{8}$/g

				if(value==''){
					cb(new Error('请输入手机号'));
				}else if(!phoneReg.test(value)){
					cb(new Error('请输入有效的手机号哦'));
				}else{
					cb();
				}
			}
			var validatPass=(rule,value,cb)=>{
				var passReg=/^[\w]{6,12}$/g //6-12位数字、字母组成
				if(value==''){
					cb(new Error('请输入密码'));
				}else if(!passReg.test(value)){
					cb(new Error('请输入有效的密码哦'));
				}else{
					cb();
				}
			}
			var validatName=(rule,value,cb)=>{
				if(value==''){
					cb(new Error('请输入用户名'));
				}else if(value.length<2){
					cb(new Error('用户名不能少于两个字符哦'));
				}else{
					cb();
				}
			}
			return {
				 ...
				 registerRules:{
				 	userPhone:[{required: true, trigger: 'blur',validator:validatPhone}],
				 	userName:[{required: true, trigger: 'blur',validator:validatName}],
				 	userPass:[{required: true, trigger: 'blur',validator:validatPass}]
				 },
			}
		},
		methods:{
